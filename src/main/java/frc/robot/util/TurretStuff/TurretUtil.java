...
/**
 * Utility class for turret targeting calculations. All distances and angles are computed from the
 * turret position, not the robot center.
 */
public class TurretUtil {
  ...
  private static final double TURRET_OFFSET_X = Constants.TurretConstants.TURRET_OFFSET_X;
  private static final double TURRET_OFFSET_Y = Constants.TurretConstants.TURRET_OFFSET_Y;
  ...
  /**
   * Returns the field-relative pose of the turret given the current robot pose. The turret heading
   * is set to the robot heading (turret rotation is handled separately).
   */
  public static Pose2d getTurretPose(Pose2d robotPose) {
    return robotPose.plus(
        new Transform2d(new Translation2d(TURRET_OFFSET_X, TURRET_OFFSET_Y), new Rotation2d()));
  }
  ...
  public static Pose2d getTargetPose(TargetType target) {
    switch (target) {
      case HUB:
        return FieldUtils.getHub();
      case LEFT_PASS:
        return FieldUtils.getLeftTarget();
      case RIGHT_PASS:
        return FieldUtils.getRightTarget();
      default:
        return FieldUtils.getHub();
    }
  }
  ...
  /**
   * Returns whichever pass target (LEFT_PASS or RIGHT_PASS) is closer to the robot's current turret
   * position.
   */
  public static TargetType getNearestPassTargetType(Pose2d robotPose) {
    ...
    double distLeft = turret.getDistance(FieldUtils.getLeftTarget().getTranslation());
    double distRight = turret.getDistance(FieldUtils.getLeftTarget().getTranslation());
  ...
  /**
   * Turret angle in degrees that the turret must rotate to, relative to the robot's heading. 0° =
   * robot forward, positive = counter-clockwise.
   */
  ...
  /**
   * Computes every parameter needed to take a shot at the given target. All geometry is measured
   * from the turret position.
   *
   ...
   *
   * <ol>
   *   <li>Uses the current time-of-flight estimate to predict where the turret will be when the
   *       ball arrives (virtual turret position).
   *   <li>Computes the distance from that virtual position to the (stationary) target.
   *   <li>Looks up a new time-of-flight, shooter speed, and trajectory angle for that virtual
   *       distance.
   *   <li>Feeds the new time-of-flight back into the next iteration.
   * </ol>
   *
   * After 5 iterations the solution has converged. The final turret angle is computed from the
   * robot's <em>current</em> pose aiming at the virtual target so the shot compensates for motion
   * during flight.
   *
   ...
   */
  public static ShotSolution computeLeadShotSolution(
      Pose2d robotPose, double robotVelX, double robotVelY, TargetType target) {
    ...
    double turretAngle =
        normalizeDegrees(Math.toDegrees(leadFieldAngle - robotPose.getRotation().getRadians()));
  ...
  public static boolean isWithinShootingRange(double distanceMeters) {
    return distanceMeters >= 0.1 && distanceMeters <= 20;
  }
  ...
  public static boolean isTurretAngleReachable(double angleDegrees) {
    return angleDegrees >= Constants.TurretConstants.REVERSELIMITDEGREES
        && angleDegrees <= Constants.TurretConstants.FORWARDLIMITDEGREES;
  }
  ...
  private static HubLookUpTable.ShootingParameters getTableParams(
      double distanceMeters, TargetType target) {
    switch (target) {
      case HUB:
      ...
      case RIGHT_PASS:
        ...
        return new HubLookUpTable.ShootingParameters(
            p.shooterSpeed, p.trajectoryAngle, p.timeOfFlight);
      default:
        return hubTable.getParameters(distanceMeters);
    }
  }
